...
Texture::Texture( const unsigned char *buffer, const char *name, unsigned int width, unsigned int rows, const std::shared_ptr<Window> &container ) :
    m_container_id( container->ID ), m_path( name )
{
...
Texture::Texture( const char *path, Window &container ) :
    m_container_id( container.ID ), m_path( path )
{
    glfwMakeContextCurrent( container.ID );
    container.set_this_shader_active();
    glGenTextures( 1, &m_id );
    glBindTexture( GL_TEXTURE_2D, m_id );
...
Texture::~Texture()
{
    glfwMakeContextCurrent( m_container_id );
