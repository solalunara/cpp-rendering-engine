...
Font::Font( const char *FontPath )
{
    ...
    for ( unsigned char c = 0; c < 128; c++ )
    {
        ...
        // generate texture
        std::unique_ptr<RawTextureData> texture_data = std::make_unique<RawTextureData>( std::basic_string<unsigned char>( face->glyph->bitmap.buffer ), to_string( static_cast<int>( c ) ).c_str(), face->glyph->bitmap.width, face->glyph->bitmap.rows );
        GlobalTexture texture( std::move( texture_data ) );
